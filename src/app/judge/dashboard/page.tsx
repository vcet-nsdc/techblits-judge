...
export default function JudgeDashboard() {
  ...
  const [assignedLab, setAssignedLab] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch judge's teams to determine assigned lab
        if (token) {
          ...
          if (teamsRes.ok) {
            ...
            // Get the judge's assigned lab name from the response
            const judgeLabName = teamsData.judgeLab?.name || teamsData.judgeLab;
            if (judgeLabName && typeof judgeLabName === "string") {
              setAssignedLab(judgeLabName);
            }
          }
        }

        // Also fetch all labs for display
        ...
        const labColors = [
          "!bg-white text-black",
          "!bg-black text-white",
          "!bg-white text-[#ff1a1a]",
          "!bg-black text-[#ff1a1a]",
          "!bg-[#ff1a1a] text-white",
        ];
        const mappedLabs = labArray.map(
          (lab: { name: string }, index: number) => ({
            id: lab.name,
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-4 md:gap-6">
          ...
          <div className="h-32 w-40 comic-border -rotate-3 hidden md:flex bg-linear-to-br from-gray-900 to-black items-center justify-center rounded-lg">
            <span className="font-display text-2xl text-white drop-shadow-[2px_2px_0_#ff1a1a]">
              QUEST!
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
          {loading ? (
            <div className="col-span-full text-center py-12">
              <span className="font-display text-2xl md:text-3xl text-gray-500">
                LOADING SECTORS...
              </span>
            </div>
          ) : (
            labs
              .sort((a, b) => {
                // Show assigned lab first
                if (assignedLab && a.id === assignedLab) return -1;
                if (assignedLab && b.id === assignedLab) return 1;
                return 0;
              })
              .map((lab) => {
                const isAssigned = assignedLab === lab.id;
                return (
                  <Link
                    key={lab.id}
                    href={`/judge/lab/${encodeURIComponent(lab.id)}`}
                  >
                    <ComicCard
                      hoverEffect
                      className={`cursor-pointer ${lab.color} h-full min-h-[160px] md:min-h-[200px] flex flex-col justify-between group overflow-hidden relative ${isAssigned ? "ring-4 ring-[#ff1a1a] shadow-[0_0_20px_rgba(255,26,26,0.5)]" : "opacity-50"}`}
                    >
                      ...
                      <div className="relative z-10">
                        <span className="font-heading text-xl bg-black text-white px-3 py-1 comic-border">
                          {lab.id} {isAssigned && "⚡ YOUR LAB"}
                        </span>
                        <h2 className="font-display text-3xl sm:text-4xl md:text-6xl mt-4 drop-shadow-[2px_2px_0_#fff]">
                          {lab.name}
                        </h2>
                      </div>
                      <div className="flex justify-between items-center mt-6 md:mt-8 relative z-10 font-heading text-lg md:text-2xl bg-white text-black p-3 md:p-4 comic-border group-hover:bg-[#ff1a1a] group-hover:text-white transition-colors">
                        <span className="flex items-center gap-2">
                          <Users />{" "}
                          {isAssigned ? "Evaluate Teams" : "View Teams"}
                        </span>
                        <Map />
                      </div>
                    </ComicCard>
